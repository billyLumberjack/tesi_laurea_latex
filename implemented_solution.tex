...
Starting from a skeleton CDA, where common values for general electrocardiograms are already stored in, and a dictionary data structure, holding specific exam related data are, the final CDA document is built and ready for the EHR.\\
...
An example is displayed: the starting SCP in fig. \ref{fig:scp_view} and the resulting Clinical Document Architecture R2 document in \ref{lst:ref}, it is important to note that even if the wevaforms raw data have been lost, the PDF representation of such information is still encapsulates inside \textit{\textlangle{}nonXMLBody\textrangle{}} tag as a \textit{base64} string.

\begin{figure}[h]
    \centering
    \includegraphics[width=\textwidth,height=15cm,keepaspectratio]{img/scp_view}
