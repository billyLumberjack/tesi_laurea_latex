...
Theese components instances are directly handled by Amazon Web Service Elastic Beanstalk (details at \ref{fig:architecture}), their load balancers and resources are dinamically allocated over the current request and load configured parameters.\\
They both run on a 64-bit Windows Server 2012 R2 (with 32-bit compatibility) machine executing IIS 8.5.
\paragraph{EcgWebApp}
...
After a configuration phase, where the user is asked to provide the endpoint which the waveforms should be uploaded to, it is possible to transfer ECG-SCP (informations about SCP-ECG file format at \ref{subsection:ecgdigitaliation}) files to remote WebServices where Ecg WebReceiver (\ref{paragraph:ecgwebreceiver}) is running.
