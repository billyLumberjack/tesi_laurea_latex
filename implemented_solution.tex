...
An example is displayed: the starting SCP in fig. \ref{fig:scp_view} and the resulting Clinical Document Architecture R2 document in snippet \ref{lst:ref}. It is important to note that even if the wevaforms raw data are lost because the conversion, the PDF representation of such information is still encapsulates inside \textit{\textlangle{}nonXMLBody\textrangle{}} tag as a \textit{base64} string.

\begin{figure}[h]
    \centering
    \includegraphics[width=\textwidth,height=10cm,keepaspectratio]{img/scp_view}
