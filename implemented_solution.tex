...
To receive large files the client must firstly asks WebReceiver for an upload-session token id, then it starts sending chunks of data until the second last one, each time communicating the uploading session id, finally the client sends the very last chunks and commits uploading (for a diagram see fig.\ref{fig:holter_sequence_diagram}). Each one of the requests made by client is authenticated server-side from a basic-authentication mechanism., the user is then identified and the exam loaded to the associated bucket on Amazon Simple Storage Service (S3).
