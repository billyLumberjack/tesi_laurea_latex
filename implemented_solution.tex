...
In order to exchange data with EHR, it is necessary to convey on a single big standard for each Electronic Medical Record (EMR \ref{subsection:electronic_health_record}). The italian EHR named, Fascicolo Sanitario Elettronico (FSE) adopted HL7 Clinical Document Architecture (CDA R2) for this kind of purpose.
\subsection{CDA document}
Hl7 Clinical Document Architecture R2 is a document markup standard \cite{hl7Cda$2$}
