...
Hl7 Clinical Document Architecture R2 is a document markup standard \cite{hl7Cda$2$}, based on XML, which aim is to encode clinical observations and services with the exchange purpose.\\
The set of provided informations comprehends text, images, sounds and multimedia content.
Everything is wrapped by a \textit{$\langle ClinicalDocument \rangle$} element, which is divided in sections too: a \textit{header} and a \textit{body}.
