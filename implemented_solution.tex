...
Hl7 Clinical Document Architecture R2 is a document markup standard \cite{hl7Cda$2$}, based on XML, which aim is to encode clinical observation and services in favour of exchange.
The set of provided informations comprehends text, images, sounds and multimedia content.
The whole document is contained in a main \textit{<ClinicalDocument>} element, which is divided into sections too: a \textit{header} and a \textit{body}.
...
In the specific Cardioline scenario, heart rate activities and cardiologist conclusions are organized inside a PDF file and encapsulated into the CDA document as an unstructured blob inside the \textit{body} tag.
